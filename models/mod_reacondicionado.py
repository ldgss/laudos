...
def guardar_reacondicionado():
    try:
        ...
        reproceso = "reproceso" in request.form
        # insert 2 vars T1|T2
        ...
        reacondicionado = text("""
                    INSERT INTO
                    reacondicionado
                    (numero_unico, responsable, fecha_registro, nueva_den, observaciones, reproceso)
                    VALUES
                    (:numero_unico, :responsable, CURRENT_TIMESTAMP, :nueva_den, :observaciones, :reproceso)
                    RETURNING id;
                """
                )
